...
void memset(void* dst, int c, size_t n)
{
    ...
    while (n-- > 0)
        *p++ = cast(ubyte)c; // TODO: Is a narrowing conversion correct?
}
...
private
{
    ...
    extern (C)
    {
        // TODO: Verify that this doesn't have to be called by the GC code.
...
// TODO: This version allocated dynamic memory for the context of aliased
//       delegates.  Use a function accepting a scope delegate instead,
...
+/
// call locked if necessary
private @property auto locked(T)(scope T delegate() op)
{
    if (thread_needLock())
        synchronized (gc.lock) return op();
    else
       return op();
}
...
size_t fullcollect(void *stackTop, bool early = false)
{
    ...
    if (collect_in_progress()) {
        ...
        case os.WRes.ERROR:
            ...
            break;
        default:
            assert(false); // BUG: Added to eliminate compile warning.
        }
    }
...
Object weakpointerGet( void* p )
{
    ...
    // TODO: Added assert because return was missing.  Is this correct?
    assert(false);
}
...
void gc_init()
{
    ...
    // BUG: For some reason, using GC.init calls _d_arrayliteralT() which
    //      doesn't work in this context because it wants to allocate memory.    
    //*gc = GC.init;
    cstring.memcpy(gc, typeid(GC).init.ptr, GC.sizeof);
    initialize();
    // TODO: Verify that this doesn't need to be called by the GC code.
    //version (DigitalMars) version(OSX) {
    //    _d_osx_image_init();
    //}
...
BlkInfo gc_qalloc(size_t size, uint attrs = 0,
        PointerMap ptrmap = PointerMap.init)
{
    if (size == 0)
        return BlkInfo.init;
    return locked(() {
        assert (Invariant()); scope (exit) assert (Invariant());
        auto p = malloc(size, attrs, ptrmap.bits.ptr);
        return getInfo(p);
    });
}
