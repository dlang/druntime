...
module core.sys.windows.threadaux; // definition of
...
HRESULT function( uint SystemInformationClass, void* info, uint infoLength, uint* ReturnLength ) fnNtQuerySystemInformation;
...
alias extern(Windows)
int function( HANDLE ThreadHandle, uint ThreadInformationClass, void* buf, uint size, uint* ReturnLength ) fnNtQueryInformationThread;
...
{
    impersonate_thread(id, &rt_moduleTlsCtor);
}
...
{
    impersonate_thread(id, &rt_moduleTlsDtor);
}
