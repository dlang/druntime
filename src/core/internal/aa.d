...
{
    // The following enums needs to determine better signature for opEquals method.
    // Associative arrays allow Values with user-defined opEquals, which may take mutable this
    // or mutable argument. If, for example, Value needs and mutable argument, we must define
    // AssociativeArray.opEquals with mutable argument. However we want to define
    // AssociativeArray.opEquals with const argument if it possible.

    // isLhsValueConst is true if in equal expression "aa1 == aa2" aa1 may be const
    private enum isLhsValueConst = lhsOpEqualsArgModifier!Value == OpEqualsArg.Const;

    // isRhsValueConst is true if in equal expression "aa1 == aa2" aa2 may be const
    private enum isRhsValueConst = rhsOpEqualsArgModifier!Value == OpEqualsArg.Const;

    static if (isLhsValueConst)
    {
        private alias LeftOpEqualsArg = const(AssociativeArray);
    }
    else
    {
        private alias LeftOpEqualsArg = AssociativeArray;

    }

    static if (isRhsValueConst)
    {
        private alias RightOpEqualsArg = const(AssociativeArray);
        private alias RightValueEqualsArg = const(Unqual!Value);
    }
    else
    {
        private alias RightOpEqualsArg = AssociativeArray;
        private alias RightValueEqualsArg = Value;
    }
    ...
    //AA allows a values which have non-const opEquals, or opEquals which takes mutable rhs arg
    ...
    static if (isLhsValueConst)
    {
        bool opEquals(RightOpEqualsArg rhs) const
        {
            return opEqualsImpl(this, rhs);
        }
    }
    else
    {
        bool opEquals(RightOpEqualsArg rhs)
        {
            return opEqualsImpl(this, rhs);
        }
    }
...
private:
    ...
    static bool opEqualsImpl(LeftOpEqualsArg lhs, RightOpEqualsArg rhs)
    {
        alias LeftEntry = Unqual!(typeof(lhs.impl.buckets[0]));
        alias RightEntry = Unqual!(typeof(rhs.impl.buckets[0]));
        if (lhs.length != rhs.length) return false;
        if (!lhs.length) return true;

        foreach (LeftEntry e1; lhs.impl.buckets[lhs.impl.firstbucket .. $])
        {
            while (e1)
            {
    ...
    static struct Entry
    {
        ...
        static if(isLhsValueConst)
        {
            bool isEqualValue(ref RightValueEqualsArg value) const
            {
        ...
        {
            bool isEqualValue(ref RightValueEqualsArg value)
            {
                return this.value == value;
            }
        }
...
private template lhsOpEqualsArgModifier(T)
{
    static if(is(typeof(const(T).init == T.init)))
    {
        enum lhsOpEqualsArgModifier = OpEqualsArg.Const;
    }
    else
    {
        enum lhsOpEqualsArgModifier = OpEqualsArg.Mutable;
    }
}

private template rhsOpEqualsArgModifier(T)
{
    static if(is(typeof(T.init == const(T).init)))
    {
        enum rhsOpEqualsArgModifier = OpEqualsArg.Const;
    }
    else
    {
        enum rhsOpEqualsArgModifier = OpEqualsArg.Mutable;
    }
}
