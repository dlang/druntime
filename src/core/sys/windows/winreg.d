...
extern (Windows) {
        LONG RegCloseKey(const HKEY);
        LONG RegConnectRegistryA(LPCSTR, HKEY, PHKEY);
        LONG RegConnectRegistryW(LPCWSTR, HKEY, PHKEY);
        LONG RegCreateKeyExA(const HKEY, LPCSTR, DWORD, LPSTR, DWORD, REGSAM,
          LPSECURITY_ATTRIBUTES, PHKEY, PDWORD);
        LONG RegCreateKeyExW(const HKEY, LPCWSTR, DWORD, LPWSTR, DWORD, REGSAM,
          LPSECURITY_ATTRIBUTES, PHKEY, PDWORD);
        LONG RegDeleteKeyA(const HKEY, LPCSTR);
        LONG RegDeleteKeyW(const HKEY, LPCWSTR);
        LONG RegDeleteValueA(const HKEY, LPCSTR);
        LONG RegDeleteValueW(const HKEY, LPCWSTR);
        LONG RegEnumKeyExA(const HKEY, DWORD, LPSTR, PDWORD, PDWORD, LPSTR, PDWORD,
          PFILETIME);
        LONG RegEnumKeyExW(const HKEY, DWORD, LPWSTR, PDWORD, PDWORD, LPWSTR, PDWORD,
          PFILETIME);
        LONG RegEnumValueA(const HKEY, DWORD, LPSTR, PDWORD, PDWORD, PDWORD, LPBYTE,
          PDWORD);
        LONG RegEnumValueW(const HKEY, DWORD, LPWSTR, PDWORD, PDWORD, PDWORD, LPBYTE,
          PDWORD);
        LONG RegFlushKey(const HKEY);
        LONG RegLoadKeyA(const HKEY, LPCSTR, LPCSTR);
        LONG RegLoadKeyW(const HKEY, LPCWSTR, LPCWSTR);
        LONG RegOpenKeyExA(const HKEY, LPCSTR, DWORD, REGSAM, PHKEY);
        LONG RegOpenKeyExW(const HKEY, LPCWSTR, DWORD, REGSAM, PHKEY);
        LONG RegQueryInfoKeyA(const HKEY, LPSTR, PDWORD, PDWORD, PDWORD, PDWORD,
          PDWORD, PDWORD, PDWORD, PDWORD, PDWORD, PFILETIME);
        LONG RegQueryInfoKeyW(const HKEY, LPWSTR, PDWORD, PDWORD, PDWORD, PDWORD,
          PDWORD, PDWORD, PDWORD, PDWORD, PDWORD, PFILETIME);
        LONG RegQueryMultipleValuesA(const HKEY, PVALENTA, DWORD, LPSTR, LPDWORD);
        LONG RegQueryMultipleValuesW(const HKEY, PVALENTW, DWORD, LPWSTR, LPDWORD);
        LONG RegQueryValueExA(const HKEY, LPCSTR, LPDWORD, LPDWORD, LPBYTE, LPDWORD);
        LONG RegQueryValueExW(const HKEY, LPCWSTR, LPDWORD, LPDWORD, LPVOID, LPDWORD);
        LONG RegReplaceKeyA(const HKEY, LPCSTR, LPCSTR, LPCSTR);
        LONG RegReplaceKeyW(const HKEY, LPCWSTR, LPCWSTR, LPCWSTR);
        LONG RegSaveKeyA(const HKEY, LPCSTR, LPSECURITY_ATTRIBUTES);
        LONG RegSaveKeyW(const HKEY, LPCWSTR, LPSECURITY_ATTRIBUTES);
        LONG RegSetKeySecurity(const HKEY, SECURITY_INFORMATION, PSECURITY_DESCRIPTOR);
        LONG RegSetValueExA(const HKEY, LPCSTR, DWORD, DWORD, const(BYTE)*, DWORD);
        LONG RegSetValueExW(const HKEY, LPCWSTR, DWORD, DWORD, const(BYTE)*, DWORD);
        LONG RegUnLoadKeyA(const HKEY, LPCSTR);
        LONG RegUnLoadKeyW(const HKEY, LPCWSTR);
        LONG RegNotifyChangeKeyValue(const HKEY, BOOL, DWORD, HANDLE, BOOL);
        ...
        deprecated {
                ...
                LONG RegEnumKeyA(const HKEY, DWORD, LPSTR, DWORD);
                LONG RegEnumKeyW(const HKEY, DWORD, LPWSTR, DWORD);
                LONG RegOpenKeyA(HKEY, LPCSTR, PHKEY);
        }
                // ADR FIXME: scheduled for deprecation once Phobos is fixed
