...
struct IN6_ADDR {
        union {
                ...
                u_short[8]      _S6_u16;
                u_long[4]       _S6_u32;
                // for old D winsock compatibility
                ubyte[16]       s6_addr;
        }
}
...
extern(Windows) {
        INT getnameinfo(const SOCKADDR* pSockaddr, socklen_t SockaddrLength,
                PCHAR pNodeBuffer, DWORD NodeBufferSize, PCHAR pServiceBuffer,
                DWORD ServiceBufferSize, INT Flags);

        static if (_WIN32_WINNT >= 0x502) {
                INT GetNameInfoW(const SOCKADDR* pSockaddr, socklen_t SockaddrLength,
