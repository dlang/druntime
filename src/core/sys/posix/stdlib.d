...
version( linux )
{
    ...
    c_long a64l(in char*);
    @trusted double drand48();
    char*  ecvt(double, int, int *, int *); // LEGACY
    @trusted double erand48(ref ushort[3]);
    char*  fcvt(double, int, int *, int *); // LEGACY
    char*  gcvt(double, int, char*); // LEGACY
    int    getsubopt(char**, in char**, char**);
    @trusted int    grantpt(int);
    char*  initstate(uint, char*, size_t);
    @trusted c_long jrand48(ref ushort[3]);
    char*  l64a(c_long);
    @trusted void   lcong48(ref ushort[7]);
    @trusted c_long lrand48();
    char*  mktemp(char*); // LEGACY
    //int    mkstemp(char*);
    @trusted c_long mrand48();
    @trusted c_long nrand48(ref ushort[3]);
    @trusted int    posix_openpt(int);
    char*  ptsname(int);
    int    putenv(char*);
    @trusted c_long random();
    char*  realpath(in char*, char*);
    @trusted ushort seed48(ref ushort[3]);
    void   setkey(in char*);
    char*  setstate(in char*);
    @trusted void   srand48(c_long);
    @trusted void   srandom(uint);
    @trusted int    unlockpt(int);
